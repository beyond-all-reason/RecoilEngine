...
function gadget:AttackCommandMovement(unitID)
	...
	if active.blocker == "clear" and (active.weapon == "corak" or active.weapon == "cormort") then
		-- Preserve #3328 when adapting the native checks to query-only masks.
		-- An underground source blocks pre-aim checks, but noGround can exclude
		-- it in a hypothetical query without changing the real weapon flags.
		local below = { Spring.TestUnitAttackMovementPosition(unitID, 1, x, y - 80, z, 0, 0) }
		local ignore = flags.noGround + flags.noUnits + flags.noFeatures + flags.noCloaked
		local filtered = { Spring.TestUnitAttackMovementPosition(unitID, 1, x, y - 80, z, 0, ignore) }
		assert(below[3] == "terrain" and below[4] == nil and below[5] == nil, "underground source must block")
		assert(filtered[2] and filtered[3] == "clear", "noGround must exclude underground source in pre-aim query")
	end
