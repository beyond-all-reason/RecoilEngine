...
with (a.root / 'console.log').open('w') as log:
    result = subprocess.run([str(a.engine), '--only-local', '--isolation', '--write-dir', str(a.root), str(a.input)], stdout=log, stderr=subprocess.STDOUT, timeout=600)
log = (a.root / 'infolog.txt').read_text(errors='replace')
for line in log.splitlines():
    if '[AttackValidation]' in line or '[AttackValidationDispatch]' in line:
        print(line)
