...
`$<textureName>` -- There are also named textures, some examples include `$minimap` `$groundfx` though there are many more, the names of which can be found in {{% repo "blob/3206e7ea9b3a043b736bcda3d581f41737914c65/rts/Lua/LuaOpenGLUtils.cpp#L53" "LuaOpenGLUtils.cpp (GetLuaMatTextureType)" %}}. ssmf_splat_normals, extra and info also have subtextures beneath though there is no central reference for these.
