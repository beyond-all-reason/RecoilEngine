...
## About the article
The article provides generic information to use GLTF models instead or in addition to the supported Assimp model (.dae) and s3o models. In the articles we don't touch the subjects of modeling, texturing, skinning or animation. Only the technicallities of Blender GLTF exporter and Recoil importer of GLTF.
The guide is written in prescriptive way. Try to not deviate from the guide too much, otherwise you may end up in the unsupported territory.
