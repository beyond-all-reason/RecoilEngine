...
//		LOG("[GR::%s][3][SDL_SetWindowFullscreen] err=\"%s\"", __func__, SDL_GetError());
	...
	if (fullScreen && !borderless) {
		...
		if (SDL_GetClosestFullscreenDisplayMode(display, newRes.x, newRes.y, 0.0f, false, &mode)) {
			SDL_SetWindowFullscreenMode(window, &mode);
		} else {
			LOG_L(L_WARNING, "[GR::%s] no fullscreen mode found for %dx%d: %s", __func__, newRes.x, newRes.y, SDL_GetError());
