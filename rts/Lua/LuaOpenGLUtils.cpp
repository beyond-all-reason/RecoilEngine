...
bool LuaOpenGLUtils::ParseTextureImage(lua_State* L, LuaMatTexture& texUnit, const std::string& image)
{
	...
	case '%': return ParseUnitTexture(texUnit, image);
...
#define udGeomBuff (unitDrawer->GetGeometryBuffer())
	...
	switch (type) {
	case LUATEX_NONE: break;
	case LUATEX_NAMED: texID = CNamedTextures::GetInfo(*reinterpret_cast<const size_t*>(&data))->id; break;
	...
	// font textures
	case LUATEX_FONT: texID = font->GetTexture(); break;
	case LUATEX_FONTSMALL: texID = smallFont->GetTexture(); break;
