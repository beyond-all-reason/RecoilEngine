...
inline size_t StablePosAllocator<T>::Allocate(size_t numElems)
{
	...
	if (positionToSize.empty()) {
		...
		// Cast thread ID to uintptr_t for logging (platform-safe)
		uintptr_t threadId = reinterpret_cast<uintptr_t>(Threading::GetCurrentThreadId());
		myLog("StablePosAllocator<T>::Allocate(%u) = %u [thread_id = %llu]", uint32_t(numElems), uint32_t(returnPos), threadId);
		return returnPos;
	}
