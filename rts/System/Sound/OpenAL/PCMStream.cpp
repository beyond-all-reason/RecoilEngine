...
bool OpenALPCMStream::FillBuffer(ALuint buffer)
{
	...
	std::int64_t deltaUs = block.ptsUs - nextQueuedPtsUs;
	if (deltaUs > 1000000) {
		const int deferredSampleRate = block.sampleRate;
		const int deferredChannels = block.channels;
		{
			std::lock_guard<std::mutex> lock(mutex);
			queuedBytes += block.samples.size() * sizeof(std::int16_t);
			blocks.emplace_front(std::move(block));
		}
		block.sampleRate = deferredSampleRate;
		block.channels = deferredChannels;
		block.ptsUs = nextQueuedPtsUs;
		block.samples.resize(static_cast<std::size_t>(sampleRate) * block.channels, 0);
		deltaUs = 0;
	}
	if (deltaUs > 0) {
		const std::size_t silenceSamples = static_cast<std::size_t>(deltaUs * block.sampleRate / 1000000) * block.channels;
		block.samples.insert(block.samples.begin(), silenceSamples, 0);
	} else if (deltaUs < 0) {
		const std::size_t blockFrames = block.samples.size() / block.channels;
		const std::int64_t blockDurationUs = blockFrames * 1000000LL / block.sampleRate;
		const std::size_t overlapSamples = (-deltaUs >= blockDurationUs)
			? block.samples.size()
			: static_cast<std::size_t>((-deltaUs) * block.sampleRate / 1000000) * block.channels;
		block.samples.erase(block.samples.begin(), block.samples.begin() + overlapSamples);
	}
	if (block.samples.empty())
		return false;
	nextQueuedPtsUs += (block.samples.size() / block.channels) * 1000000LL / block.sampleRate;
...
void OpenALPCMStream::Update(bool suspended)
{
	...
	alGetSourcei(source, AL_BUFFERS_QUEUED, &queued);
	if (endOfStream && queued == 0 && bufferSamples.empty()) {
		std::lock_guard<std::mutex> lock(mutex);
		drained = blocks.empty();
	}
}
