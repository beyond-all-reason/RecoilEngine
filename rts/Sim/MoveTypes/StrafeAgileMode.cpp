...
// the gusts: two octaves of noise, their periods as shares of the oscillator's
static constexpr float HOVER_GUST_GAIN = 0.8f;
static constexpr float HOVER_GUST_SECOND_OCTAVE_GAIN = 0.6f;
static constexpr float HOVER_GUST_PERIOD_SHARES[2] = {0.4f, 0.875f};
...
// smooth noise in [-1, 1] with a new value every <period> frames, a pure function
// of its arguments: every aircraft gets its own gusts without drawing from the synced RNG
static float GustNoise(unsigned int seed, int frame, int period)
{
	// (an integer hash of the seed and the knot, its low 16 bits mapped to [-1, 1])
	const auto Knot = [seed](unsigned int knot) {
		unsigned int h = (seed * 0x9E3779B1u) ^ (knot * 0x85EBCA77u);

		h ^= (h >> 15); h *= 0x2C1B3C6Du;
		h ^= (h >> 12); h *= 0x297A2D39u;
		h ^= (h >> 15);

		return ((h & 0xFFFF) * (2.0f / 0xFFFF) - 1.0f);
	};

	const unsigned int knot = frame / period;

	return (mix(Knot(knot), Knot(knot + 1), smoothstep(0.0f, 1.0f, float(frame % period) / period)));
}
...
float3 CStrafeAirMoveType::UpdateHoverSway(bool holding, float3& leanAcc)
{
	...
	const int gustPeriods[2] = {std::max(1, int(period * HOVER_GUST_PERIOD_SHARES[0])), std::max(1, int(period * HOVER_GUST_PERIOD_SHARES[1]))};
	...
	for (int axis = 0; axis < 3; axis++) {
		const unsigned int seed = owner->id * 8 + axis * 2;

		gust[axis] = HOVER_GUST_GAIN * (GustNoise(seed, gs->frameNum, gustPeriods[0]) + HOVER_GUST_SECOND_OCTAVE_GAIN * GustNoise(seed + 1, gs->frameNum, gustPeriods[1]));
	}
